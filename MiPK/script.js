...
function activarCapaTrazado() {
    console.log("Activar Capa Trazado (con líneas cada 2km) - DEBUGGING"); // Mensaje de inicio de debugging
    if (trazadoLayer) {
    ...
    cargarArchivosJSON(rutasArchivos)
        .then(dataTrazado => {
            console.log('dataTrazado cargado. Longitud:', dataTrazado.length); // Información sobre los datos cargados
            if (!dataTrazado || !Array.isArray(dataTrazado)) {
                console.error("Error: dataTrazado no es un array válido."); // Error si dataTrazado no es un array
                return; // Salir de la función si hay un problema con dataTrazado
            }

            dataTrazado.forEach((punto, index) => { // Añadimos el índice para identificar puntos
                console.log(`\nProcesando punto ${index}:`, punto); // Imprime cada punto ANTES de procesarlo

                if (!punto || typeof punto !== 'object') {
                    console.warn(`Punto ${index} no es un objeto válido. Ignorando.`); // Advertencia si el punto no es un objeto
                    return; // Saltar este punto si no es un objeto válido
                }

                const latitud = parseFloat(punto.Latitud);
                const longitud = parseFloat(punto.Longitud);

                if (isNaN(latitud) || isNaN(longitud)) {
                    console.error(`Error en punto ${index}: Latitud o Longitud no son números válidos. Punto:`, punto); // Error si Latitud o Longitud no son números
                ...
                if (!lastPoint) {
                    ...
                    try {
                        const distance = mapa.distance(lastPoint, latlng); // Calcular distancia - POSIBLE PUNTO DE ERROR
                        ...
                        console.error(`Error al calcular distancia en punto ${index}:`, punto, distanceError); // Error específico al calcular la distancia
                    }
                }
            });
            ...
            mapa.addLayer(trazadoLayer);
            console.log("Capa Trazado activada (con líneas cada 2km) - DEBUGGING FIN"); // Mensaje de fin de debugging
        })
        .catch(error => {
            console.error("Error general al activar la capa de trazado:", error); // Error general de la función
        });
}
